...
my.analysis<-lm(log(Ozone)~Solar.R+Wind+Temp+
  ...
  #Expand the linear model from Exercise 11.1 
  #with pairwise interactions between solar radiation, wind and temperature.
                data=airquality[airquality>1,])

summary(my.analysis)

#Reduce the model with the drop1() function, 
#removing insignificant factors sequentially, 
#by removing the one with the biggest p-value (above 0.05).
...
drop1(my.analysis,test="F")
#Now, the main effect Wind is a removeable factor, 
#since it now doesn’t interact with anything. 
#However, both Wind and the interaction term Solar.R:Temp #
#are statistically significant at the 5% level, 
#therefore, we will not reduce the model further. 
#With a p-value of 0.02 for the interaction term Solar.R:Temp, 
#the data therefore supports the presence of interaction terms.
